...
// #include "im2d.h"  // 暂时移除，未找到头文件
// #include "drmrga.h"

// stb_image.h 需要单独下载，暂时禁用
...
// #include "dma_alloc.h"  // 暂时移除，未找到头文件
...
// turbojpeg.h 需要单独下载，暂时禁用
#define DISABLE_LIBJPEG
#if 0  // 禁用 turbojpeg 相关代码
#ifndef DISABLE_LIBJPEG
// #include "turbojpeg.h"  // 暂时移除，未找到头文件
...
// read_image_jpeg 需要 turbojpeg.h，暂时禁用
#if 0
...
#endif  // #if 0 - read_image_jpeg 需要 turbojpeg.h

// write_image_jpeg 需要 turbojpeg.h，暂时禁用
#if 0
...
#endif  // #if 0 - write_image_jpeg 需要 turbojpeg.h
#endif  // #if 0 - 禁用 turbojpeg 相关代码
...
// read_image_stb 函数需要 stb_image.h，暂时禁用
#if 0
...
#endif  // #if 0 - read_image_stb 需要 stb_image.h
...
int read_image(const char* path, image_buffer_t* image)
{
    ...
    if (strcmp(_ext, ".data") == 0) {
        ...
        // read_image_stb 需要 stb_image.h，暂时返回错误
        printf("error: read_image_stb not available (stb_image.h not found)\n");
        return -1;
...
int write_image(const char* path, const image_buffer_t* img)
{
    ...
    if (strcmp(_ext, ".png") == 0 | strcmp(_ext, ".PNG") == 0) {
        // stbi_write_png 需要 stb_image_write.h，暂时返回错误
        printf("error: stbi_write_png not available (stb_image_write.h not found)\n");
        ret = -1;
...
#else
        // stbi_write_jpg 需要 stb_image_write.h，暂时返回错误
        printf("error: stbi_write_jpg not available (stb_image_write.h not found)\n");
        ret = -1;
        // ret = stbi_write_jpg(path, width, height, channel, data, quality);
#endif
    } else if (strcmp(_ext, ".data") == 0 | strcmp(_ext, ".DATA") == 0) {
        int size = get_image_size((image_buffer_t*)img);  // 移除 const 限定符
...
static int convert_image_cpu(image_buffer_t *src, image_buffer_t *dst, image_rect_t *src_box, image_rect_t *dst_box, char color) {
    ...
    if (src->virt_addr == NULL) {
        LOGE("src buffer is null\n");
        return -1;
    }
    // if (src->format != dst->format) {
    //     LOGE("src and dst format is not same\n");
    //     return -1;
    // }
    // [1] 新增独立缓冲区处理格式转换
    image_buffer_t tmp_src;
    unsigned char* converted_buffer = NULL;
    if (src->format == IMAGE_FORMAT_RGBA8888 && 
        dst->format == IMAGE_FORMAT_RGB888) 
    {
        // 分配独立转换缓冲区
        int src_pixels = src->width * src->height;
        converted_buffer = (unsigned char*)malloc(src_pixels * 3);
        if (!converted_buffer) {
            LOGE("内存分配失败: RGBA→RGB转换缓冲区");
            if (converted_buffer) {
        ...
        // 执行通道转换
        unsigned char* rgba_ptr = src->virt_addr;
        unsigned char* rgb_ptr = converted_buffer;
        ...
        // 创建临时源参数
        tmp_src.virt_addr = converted_buffer;
        tmp_src.width = src->width;
        tmp_src.height = src->height;
        tmp_src.format = IMAGE_FORMAT_RGB888;
        src = &tmp_src;
    }
    else if (src->format != dst->format) {
        LOGE("格式不匹配: src=%d, dst=%d", src->format, dst->format);
        if (converted_buffer) {
            free(converted_buffer);
            converted_buffer = NULL;
        }
...
// get_rga_fmt 需要 RGA 格式定义，暂时禁用
#if 0
static int get_rga_fmt(image_format_t fmt) {
    switch (fmt)
    {
    case IMAGE_FORMAT_RGB888:
        return RK_FORMAT_RGB_888;  // 需要 im2d.h 中的定义
    case IMAGE_FORMAT_RGBA8888:
        return RK_FORMAT_RGBA_8888;  // 需要 im2d.h 中的定义
    case IMAGE_FORMAT_YUV420SP_NV12:
        return RK_FORMAT_YCbCr_420_SP;  // 需要 im2d.h 中的定义
    case IMAGE_FORMAT_YUV420SP_NV21:
        return RK_FORMAT_YCrCb_420_SP;  // 需要 im2d.h 中的定义
    default:
        return -1;
    }
}
...
// convert_image_rga 函数需要 im2d.h，暂时禁用
#if 0
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    // LOGD("use_handle=%d",use_handle);
    use_handle = 1;
    if (srcFmt < 0 || dstFmt < 0) {
        LOGE("Unsupported format: src_fmt=%d, dst_fmt=%d", 
             src_img->format, dst_img->format);
        return -1;
    }
    if ((uintptr_t)src % 16 != 0 || (uintptr_t)dst % 16 != 0) {
        LOGE("Unaligned memory address: src=%p dst=%p", src, dst);
        return -1;
    }
    ...
    if (drect.width != dstWidth || drect.height != dstHeight) {
        ...
        LOGD("fill dst image (x y w h)=(%d %d %d %d) with color=0x%x\n",
            dst_whole_rect.x, dst_whole_rect.y, dst_whole_rect.width, dst_whole_rect.height, imcolor);
        ...
        if (ret_rga <= 0) {
            if (dst != NULL) {
                ...
                LOGE("Warning: Can not fill color on target image\n");
            }
        }
    }
...
#endif  // #if 0 - convert_image_rga 需要 im2d.h
...
//         ret = dma_buf_alloc(DMA_HEAP_DMA32_UNCACHE_PATCH, src_img->size, &src_dma_fd, &src_virt);
//         if (ret < 0) {
//             LOGE("Alloc src DMA buffer failed! size=%d", src_img->size);
//             goto CLEAN_ALL;
//         }
//         memcpy(src_virt, src_img->virt_addr, src_img->size);
//         src_img->fd = src_dma_fd;
//     }

//     // 分配目标图像DMA缓冲区
//     if (dst_img->virt_addr == NULL) {
//         ret = dma_buf_alloc(DMA_HEAP_DMA32_UNCACHE_PATCH, dst_img->size, &dst_dma_fd, &dst_virt);
//         if (ret < 0) {
//             LOGE("Alloc dst DMA buffer failed! size=%d", dst_img->size);
//             goto CLEAN_ALL;
//         }
...
//         .format = get_rga_fmt(src_img->format)
//     };

//     im_handle_param_t dst_param = {
//         .width = dst_img->width,
//         .height = dst_img->height,
//         .format = get_rga_fmt(dst_img->format)
//     };

//     // 创建RGA缓冲区句柄
//     src_handle = importbuffer_fd(src_img->fd, &src_param);
//     dst_handle = importbuffer_fd(dst_img->fd, &dst_param);
//     if (src_handle <= 0 || dst_handle <= 0) {
//         LOGE("Import buffer failed! src_handle=%d, dst_handle=%d", src_handle, dst_handle);
//         ret = -1;
//         goto CLEAN_ALL;
//     }
...
//                                            src_img->width,
//                                            src_img->height,
//                                            get_rga_fmt(src_img->format));

//     rga_buffer_t dst_buf = wrapbuffer_handle(dst_handle,
//                                            dst_img->width,
//                                            dst_img->height,
//                                            get_rga_fmt(dst_img->format));

//     // 转换坐标到im_rect结构
//     im_rect srect = {
//         .x = src_box->left,
//         .y = src_box->top,
//         .width = src_box->right - src_box->left + 1,
//         .height = src_box->bottom - src_box->top + 1
//     };

//     im_rect drect = {
//         .x = dst_box->left,
//         .y = dst_box->top,
//         .width = dst_box->right - dst_box->left + 1,
//         .height = dst_box->bottom - dst_box->top + 1
//     };

//     // ================== 4. 执行RGA操作 ==================
//     IM_STATUS status = improcess(src_buf, dst_buf, pat, srect, drect, prect, 0);
//     if (status != IM_STATUS_SUCCESS) {
//         LOGE("RGA process failed: %s", imStrError(status));
//         ret = -1;
//     }

//     // ================== 5. 数据回写 ==================
//     if (dst_virt != NULL) {
//         dma_sync_cpu_to_device(dst_img->fd); // DMA同步
//         memcpy(dst_img->virt_addr, dst_virt, dst_img->size);
//     }
...
//     if (src_dma_fd > 0) dma_buf_free(src_img->size, &src_dma_fd, src_virt);
//     if (dst_dma_fd > 0) dma_buf_free(dst_img->size, &dst_dma_fd, dst_virt);
...
{
    int ret;
    // 强制使用 CPU 版本，因为 RGA 需要 im2d.h
    // #if defined(DISABLE_RGA) 
    printf("convert image use cpu\n");
    LOGD("convert image use cpu\n");
...
#endif
        // LOGD("src width is 4/16-aligned, convert image use rga\n");
        // ret = convert_image_rga(src_img, dst_img, src_box, dst_box, color);
        LOGD("使用cpu转换图像\n");
        ret = convert_image_cpu(src_img, dst_img, src_box, dst_box, color);
        if (ret != 0) {
            printf("try convert image use cpu\n");
            LOGD("try convert image use cpu\n");
            ret = convert_image_cpu(src_img, dst_img, src_box, dst_box, color);
        }
    } else {
        printf("src width is not 4/16-aligned, convert image use cpu\n");
        LOGD("src width is not 4/16-aligned, convert image use cpu\n");
        ret = convert_image_cpu(src_img, dst_img, src_box, dst_box, color);
    }
#endif  // #if 0 - 禁用 RGA 代码
    return ret;
}
...
int convert_image_with_letterbox(image_buffer_t* src_image, image_buffer_t* dst_image, letterbox_t* letterbox, char color)
{
    ...
    LOGD("Input image format: %d, expected RGBA8888(%d)", src_image->format, IMAGE_FORMAT_RGBA8888);
    if (src_image->format != IMAGE_FORMAT_RGBA8888) {
        LOGE("Unsupported input format! Require RGB8888");
        return -1;
    }
    ...
    if (ret < 0) {
        LOGE("convert_image failed! ret=%d, src=%dx%d(fmt:%d), dst=%dx%d(fmt:%d)", 
             ret, 
             src_image->width, src_image->height, src_image->format,
             dst_image->width, dst_image->height, dst_image->format);
        return -1;
    }
    return ret;
}