...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./model/full_2_labels_list.txt"
...
[[maybe_unused]] static float sigmoid(float x) { return 1.0 / (1.0 + expf(-x)); }

[[maybe_unused]] static float unsigmoid(float y) { return -1.0 * logf((1.0 / y) - 1.0); }
...
[[maybe_unused]] static int process_u8(uint8_t *box_tensor, int32_t box_zp, float box_scale,
...
{
    static char filtered_name[64]; // 线程不安全但简单有效
    static char null_name[] = "null";
    memset(filtered_name, 0, sizeof(filtered_name));
    ...
    if (labels[cls_id])
    {
        ...
        // 过滤控制字符
        const char* src = labels[cls_id];
        char* dst = filtered_name;
        while (*src && (dst - filtered_name) < 63) {
            if (isprint((unsigned char)*src)) { // 仅保留可打印字符
                *dst++ = *src;
            }
            src++;
        }
