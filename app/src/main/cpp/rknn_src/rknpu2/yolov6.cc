...
void log_memory(const char* tag){
    ...
    LOGD("[%s] 内存使用: %zu KB", tag, mi.uordblks / 1024);
}
...
int init_yolov6_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    ...
    if (model == NULL)
    {
        ...
        if (model) free(model); //释放模型数据
        if (ret < 0){           //错误路径额外清理
            if (ctx) rknn_destroy(ctx);//释放RKNN上下文
    ...
    // memset(input_attrs, 0, sizeof(input_attrs)); // Not needed for vector
    ...
    // memset(output_attrs, 0, sizeof(output_attrs)); // Not needed for vector
...
int init_yolov6_model_from_memory(const char *model_data, int model_size, rknn_app_context_t *app_ctx)
{
    int ret = 0;
    rknn_context ctx = 0;
    app_ctx->input_attrs = NULL;  // 显式初始化
...
int inference_yolov6_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results)
{
    ...
    // memset(inputs, 0, sizeof(inputs)); // vector不需要memset
    // memset(outputs, 0, sizeof(outputs)); // vector不需要memset
    std::fill(inputs.begin(), inputs.end(), rknn_input{});
    ...
    // letterbox
    LOGD("yolov6_format:%d",img->format);
    log_memory("图像处理开始");
